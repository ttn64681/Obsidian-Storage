...
- SQL defines the methods used to create and manipulate relational databases on all major platforms
	- **Normalization**
		- database design technique - *reduces data redundancy* and *eliminates undesirable characteristics* like insertion, update, and deletion *anomalies*.
		- Normalization rules divide larger tables into smaller tables, and link them using relationships
		-<u> Goal: Eliminate redundant data + ensure data is stored logically</u>
	- **SQL Commands  (2 main sub-langs): DDL and DML** 
		- *Data Definition Lang* - contains commands used to create and destroy databases and database objects
		- After database structure is defined w/ DDL, database admins and users can use... =>
		- *Data Manipulation Lang* - to insert, retrieve, and modify data contained w/in it
		- *Database Joins* - join statement combines data in several tables to efficiently process large quantities of data<u> (5 types of joins)</u>:
		- Inner joins
		- Outer joins
		- Right joins
		- Left joins
...
## Cloud Fundamentals
- refers to various forms of virtualized IT infrastructure
- in other words, *cloud provides extended, virtualized computing resources that are available anywhere* (even tho they run on physical components stored on-premises or at a vendor's data center)
- In this way, servers, OS software, networking, and other infrastructure can be  pooled and divided w/o regarding physical hardware boundaries
	- *3 Types of Cloud Service models:*
		- **IaaS** - (HaaS) most flexible cloud service type - gives complete control over hardware that runs app
			- server, nothing else. still have to manage OS
		- **PaaS** - provides ready-to-use development env where devs can focus on writing and executing code to make customized apps 
			- deployment and maintenance service, AWS Elastic Beanstalk
		- **SaaS** - provides complete product that is fully run and managed by service provider
			- e.g. Google Mail
![[Pasted image 20250818230634.png]]
- 
	- *3 Types of Cloud Deployment models:*
		- **Public** - virtual server, free or pay-per-use
		- **Private** - proprietary env dedicated to single business or org
		- **Hybrid** - both, where subscriber chooses

# Refreshing Essential Development Skills
- Summarize basic softdev concepts
- Define fundamental softdev terms
- Determine if further review or study is needed

## Web coding skills
- front, back, and full stack devs
- front-end langs and back-end langs
	- front-end:
		- HTML, CSS, JS, React
	- back-end:
		- Python, Django, and Flask
	- Frameworks and libraries extend functionality of both types of coding lang, including JS and Python
